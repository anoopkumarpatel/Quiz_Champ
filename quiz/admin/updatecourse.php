...
<div class="col-sm-6">default.php
<h3 style="color:;" class="mt-2" style="margin-left:200px;"><b>Update</b> Course
...
<b>Logout</b>
   ...
   <div class="col-sm-10 cont">
    <?php
$id=$_REQUEST['id'];
$query2="select * from ourcources where id='$id'";
...
if($row=mysqli_fetch_array($res))
{
	?>
	 ...
	 <b style="font-weight:bold;color:white;margin-top:0px;letter-spacing:4px;font-size:20px;">Description:</b>
	    <input type="text" class="form-control" name="desc" required="true" value="<?PHP echo $row['descr'];?>"/><br/>
		<div class="row">
		    <div class="col-sm-6">
			  <b style="font-weight:bold;color:white;margin-top:0px;letter-spacing:4px;font-size:17px;">Course :</b>
	    <input type="text" class="form-control" name="course" required="true" value="<?PHP echo $row['cources'];?>"/>
			</div>
		...
		<br/>
		<br/>
		<input type="submit" value="Update+ >>" name="updatecourse" class="btn btn-success" style="width:300px;height:40px;float:right;font-weight:bold;font-size:20px;">
	 </form>
	 <?php
}
?>	
...
include("../connection.php");
 $query3="update ourcources set cources='$course',descr='$desc',addby='$name',date='$date' where id='$id'";
      mysqli_query($conn,$query3);
echo"<script>alert('Course Update Successfully');window.location.href='coursemgmt.php';</script>";
 }
?>
