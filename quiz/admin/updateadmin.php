...
<div class="col-sm-6">
<h3 class="mt-2" style="margin-left:200px;"><b>Update</b> Admin
</h3>
</div>
<div class="col-sm-2">
...
if($row=mysqli_fetch_array($res))
{
	?>
	 ...
	 <b style="font-weight:bold;color:white;margin-top:0px;letter-spacing:4px;font-size:20px;">Admin name:</b>
	    <input type="text" class="form-control" name="txtname" required="true" value="<?PHP echo $row['name'];?>"/><br/>
		<div class="row">
		    <div class="col-sm-6">
			  <b style="font-weight:bold;color:white;margin-top:0px;letter-spacing:4px;font-size:17px;">ID:</b>
	    <input type="text" class="form-control" name="txtid" required="true" value="<?PHP echo $row['userid'];?>" readonly="true"/>
			</div>
			<div class="col-sm-6">
			  <b style="font-weight:bold;color:white;margin-top:0px;letter-spacing:4px;font-size:17px;">Password:</b>
	    <input type="text" class="form-control" name="txtpass" required="true" value="<?PHP echo $row['password'];?>"/>
			</div>
		</div>
		<br/>
		<br/>
		...
		<br/>
		<br/>
		<input type="submit" value="UPDATE >>" name="updateadmin" class="btn btn-success" style="width:300px;height:40px;float:right;font-weight:bold;font-size:20px;">
...
<?PHP
 if(isset($_POST['updateadmin']))
 {
	 ...
	 $query3="update admin set name='$name',password='$password',date='$date' where userid='$id'";
      mysqli_query($conn,$query3);
echo"<script>alert('Admin Update Successfully');window.location.href='adminmgmt.php';</script>";
 }
?>